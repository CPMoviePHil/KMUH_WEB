...
$account = $password = $password_hashed = $username = $roles = "";
$account_err = $password_err = $password_hased_err = $username_err = $roles_err = "";
$username_data = $account_data = array();
  ...
  if($_SESSION['status'] == 'add'){
    if($_SERVER["REQUEST_METHOD"] == "POST"){
      if(empty(trim($_POST['account']))){
        ...
        $selectsql = "SELECT userid FROM accounts WHERE userid = ?";

        if($stmt = mysqli_prepare($link, $selectsql)){
          mysqli_stmt_bind_param($stmt, 's', $param_userid);
          $param_userid = trim($_POST['account']);
      ...
      if(empty($account_err) && empty($password_err) && empty($username_err) && empty($roles_err)){
       $insert1 = "INSERT INTO accounts (userid, password, username, role) VALUES (?,?,?,?)";
       $insert2 = "INSERT INTO adminuser (username, userid) VALUES (?,?)";
       $insert3 = "INSERT INTO adminuserinformation (userid) VALUES (?)";
       
       if($stmt = mysqli_prepare($link, $insert1)){
        mysqli_stmt_bind_param($stmt, 'ssss', $param_userid, $param_password, $param_username, $param_role);
...
if($_SESSION['status'] == 'edit'){
  if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST['account']))){
      ...
      if($_SESSION['account'] == trim($_POST['account']) ){
    ...
    if(empty($account_err) && empty($password_err) && empty($username_err) && empty($roles_err)){
     $update1 = "UPDATE accounts SET userid = ?, password=?, username = ?, role =? WHERE userid =?";
     $update2 = "UPDATE adminuser SET username = ? , userid = ? WHERE userid = ?";
     $update3 = "UPDATE adminuserinformation SET userid = ? WHERE userid = ?";
     if($stmt = mysqli_prepare($link, $update1)){
      mysqli_stmt_bind_param($stmt, 'sssss', $param_update_userid, $param_password, $param_username, $param_role, $param_userid);
      $param_update_userid = $account;
      $param_password = $password;
      $param_username = $username;
      $param_role = $roles;
      $param_userid = $_SESSION['account'];
      if(mysqli_stmt_execute($stmt)){

      }else
      {
        echo 'really fuck kkkkkk me';
      }
    }
    mysqli_stmt_close($stmt);
    if($stmt = mysqli_prepare($link, $update2)){
      mysqli_stmt_bind_param($stmt, 'sss', $param_username, $param_update_userid, $param_userid);
      $param_update_userid = $account;
      $param_username = $username;
      $param_userid = $_SESSION['account'];
      if(mysqli_stmt_execute($stmt)){
    ...
    mysqli_stmt_close($stmt);
    if($stmt = mysqli_prepare($link, $update3)){
      mysqli_stmt_bind_param($stmt, 'ss', $param_update_userid, $param_userid);
      $param_update_userid = $account;
      $param_userid = $_SESSION['account'];
      if(mysqli_stmt_execute($stmt)){
      }
      else{
        echo 'fuck yyyyyyyeah!';
      }
    }
    mysqli_stmt_close($stmt);
    }//POST
...
$getdata = "SELECT username, userid FROM adminuser";
if($stmt = mysqli_prepare($link, $getdata)){
  if(mysqli_stmt_execute($stmt)){
    mysqli_stmt_bind_result($stmt, $temp_username, $temp_account);
  ...
  <div class="container body">
    <div class="main_container">
      ...
      <div class="right_col" role="main">
        ...
        <div class="page_title"><h3>管理員資料</h3></div>
        <div id="thereplace">
          <div class="button_to_add">
            <?php
            if($pofadd[1]){
              echo '<a class="btn btn-primary" data-ajax="true" data-ajax-method="POST" data-ajax-mode="replace" data-ajax-update="#thereplace" href="adminuser/addadminuser.php">新增管理員帳號</a>';
            }
            ?>
            <span class="help-block"><?php echo $account_err.' '.$password_err.' '.$username_err.' '.$roles_err?></span>
          </div>
          ...
          <tbody>
            <?php 
            foreach(array_combine($username_data, $account_data) as $vals1 => $vals2){
              echo "<tr>"."<td>".$vals1."</td>"."<td>".$vals2."</td>"."<td>";
              if($pofedit[1]){
                echo "<a class='btn btn-warning' data-ajax='true' data-ajax-method='POST' data-ajax-mode='replace' data-ajax-update='#thereplace' href='adminuser/editadminuser.php?val=".$vals2."'>"."修改帳密權限"."</a>";
              }
              if($pofdel[1]){
                echo "<a class='btn btn-danger' data-ajax='true' data-ajax-confirm='是否要刪除該筆資料' data-ajax-method='POST' data-ajax-success='isDeleteSuccess' href='adminuser/deladminuser.php?val=".$vals2."'>"."刪除"."</a>";
              }
              echo '</td></tr>';
            }
            ?>
